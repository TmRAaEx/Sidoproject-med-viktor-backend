...
export const fetchAndConvertLecabData = async (): Promise<ICar[]> => {
    try {
        ...
        const response = await fetch("https://store.lecab.se/api/products?limit=999", {
            method: "GET",
            headers: {
        ...
        if (response.status === 307) {
            ...
            const redirectedResponse = await fetch(`https://store.lecab.se${redirectUrl}`, {
            ...
            if (!redirectedResponse.ok) {
                throw new Error(`[lecaberror]Redirected request failed with status: ${redirectedResponse.status}`);
            }
            ...
            console.error(`[lecaberror]Initial request failed with status: ${response.status}`);
...
const convertData = (data: any[]): ICar[] => {
    return data.map((car: any): ICar => ({
        ...
        attributes: (car.attributes || []).map((attribute: any): ICarAttribute => ({
            id: attribute.id,
            name: attribute.name,
